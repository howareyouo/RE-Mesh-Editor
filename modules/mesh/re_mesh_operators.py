...
class WM_OT_RenameMeshToREFormat(Operator):
	...
	def execute(self, context):
		...
		for groupID, objList in groupDict.items():
	...
	def execute(self, context):
		...
		for obj in selection:
			if obj.type != "MESH" or len(obj.vertex_groups) == 0:
				continue
			# Single pass collect all used group indices (O(V*K) instead of O(G*V*K))
			all_groups_data = []
			for v in obj.data.vertices:
				for g in v.groups:
					all_groups_data.append(g.group)
			# Use numpy vectorized unique + isin to find empty groups
			if all_groups_data:
				used_indices = np.unique(np.array(all_groups_data, dtype=np.int32))
				vg_indices = np.array([vg.index for vg in obj.vertex_groups], dtype=np.int32)
				empty_mask = ~np.isin(vg_indices, used_indices)
				empty_vgs = [obj.vertex_groups[i] for i in np.where(empty_mask)[0]]
			else:
				empty_vgs = list(obj.vertex_groups)
			# Remove in reverse order to maintain index stability
			for vg in reversed(empty_vgs):
...
class WM_OT_LimitTotalNormalizeAll(Operator):
	...
	maxWeights: EnumProperty(
		...
		items=[("4", "4 Weights", "Safest option but potentially lower weight quality"),
		       ...
		       ("12", "12 Weights (MH Wilds or Newer)",
...
def checkForChildRECollectionsRecursive(collection):  
	...
	def invoke(self, context, event):
		...
		if self.skipPrompt:
			...
			return context.window_manager.invoke_props_dialog(self, width=EXPORTER_WINDOW_SIZE,
			                                                  confirm_text="Batch Export Files")
		...
		if self.itemList_index != -1:
			...
			if not item.hasChild and item.exportType != "":
				...
				if item.invalid:
					row = box.row()
					row.alert = True
					row.label(text="Path is empty or missing the file version number on the end. ",
					          icon="ERROR")
